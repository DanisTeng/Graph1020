...
class DiscreteDynamic1d:
    ...
    # From state, Control id, To state
    Transition = Tuple[StateId, ControlId, StateId]
    ...
    def __init__(self, dynamic1d: Dynamic1d, param: DiscreteDynamic1dParam, load_from_cache=True, dump_to_cache=True):
        ...
        assert 0.0 <= self.dynamic1d.min_speed < 1e-9, "not supporting reversed driving, or speed lower bound."
        ...
        self.a_partition = BVSPartition(self.dynamic1d.min_acc, self.dynamic1d.max_acc, self.da)
    ...
    def calculate_forward_reachable_states(self, from_state_id: StateId, control_id: ControlId) -> Set[StateId]:
        ...
        for i_v in range(num_v_sample):
            ...
            for i_a in range(num_a_sample):
                ...
                next_s, next_v, next_a = self.dynamic1d.state_transition(0.0, v, a, jerk, self.dt)
                ...
                for next_sid in range(*self.s_partition.overlapping_index_range(next_s_interval)):
