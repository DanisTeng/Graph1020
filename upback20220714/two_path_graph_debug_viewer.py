...
The debug GUI for two_path_graph
"""
...
import experimental.users.huaiyuan.regulated_safety.vehicle as vehicle
from experimental.users.huaiyuan.regulated_safety.discrete_dynamic1d import (
    DiscreteDynamic1d,
)
from experimental.users.huaiyuan.regulated_safety.path import PathBase
...
class TwoPathGraphViewer:
    ...
    class GuiState:
        def __init__(self):
            ...
            self.transition_type: str = "backward"
    ...
    class FilterPanel:
        ...
        class SinglePredFilter:
            ...
            def __init__(
                ...
                self.true_check = tk.Checkbutton(
                    mw,
                    onvalue=1,
                    offvalue=0,
                    variable=self.true_var,
                    text="True",
                    command=self.on_select_true,
                )
                self.false_check = tk.Checkbutton(
                    mw,
                    onvalue=1,
                    offvalue=0,
                    variable=self.false_var,
                    text="False",
                    command=self.on_select_false,
                )
                self.any_check = tk.Checkbutton(
                    mw,
                    onvalue=1,
                    offvalue=0,
        ...
        def __init__(self, father_window, gui_state: "TwoPathGraphViewer.GuiState"):
            ...
            for i in range(len(gui_state.graph.predicates)):
                self.predicate_filters.append(
                    self.SinglePredFilter(
                        self.frame,
                        current_row,
                        gui_state.graph.predicates[i].name(),
                        i,
                        self.gui_state,
            ...
            self.transition_type_button = tk.Button(
                self.frame, text="", command=self.on_transition_type_change
            )
            self.transition_type_button.grid(
                row=current_row, column=0, columnspan=2, sticky=tk.NSEW
            )
            self.refresh_button = tk.Button(
                self.frame, text="apply", command=self.update_by_gui_state
            )
            self.refresh_button.grid(
                row=current_row, column=2, columnspan=2, sticky=tk.NSEW
            )
        ...
        def update_by_gui_state(self):
            ...
            if is_forward:
                ...
                a1_reachable_states = a1.discrete_dynamic.backward_reachable_states(
                    s1_0
                )
                a2_reachable_states = a2.discrete_dynamic.backward_reachable_states(
                    s2_0
                )

            for s1_t in a1_reachable_states:
                if is_forward:
                    a1_jerk_set = graph.a1.discrete_dynamic.transition_controls(
                        s1_0, s1_t
                    )
                else:
                    a1_jerk_set = graph.a1.discrete_dynamic.transition_controls(
                ...
                a1_jerk_range = graph.a1.discrete_dynamic.jid_set_to_jerk_min_max(
                    a1_jerk_set
                )
                a1_disjoint = (
                    a1_jerk_range[0] > self.gui_state.a1_jerk_max
                    or a1_jerk_range[1] < self.gui_state.a1_jerk_min
                )
                if a1_disjoint:
                    continue
                ...
                for s2_t in a2_reachable_states:
                    if is_forward:
                        a2_jerk_set = graph.a2.discrete_dynamic.transition_controls(
                            s2_0, s2_t
                        )
                    else:
                        a2_jerk_set = graph.a2.discrete_dynamic.transition_controls(
                    ...
                    a2_jerk_range = graph.a2.discrete_dynamic.jid_set_to_jerk_min_max(
                        a2_jerk_set
                    )
                    a2_disjoint = (
                        a2_jerk_range[0] > self.gui_state.a2_jerk_max
                        or a2_jerk_range[1] < self.gui_state.a2_jerk_min
                    )
                    if a2_disjoint:
                        continue
                    ...
                    for true_pass_pred in self.gui_state.filter_label_white_list:
                        if not self.gui_state.graph.predicates[true_pass_pred].evaluate(
                            state_to_show
                        ):
                            pred_all_pass = False
                            break
                    for false_pass_pred in self.gui_state.filter_label_black_list:
                        if self.gui_state.graph.predicates[false_pass_pred].evaluate(
                            state_to_show
                        ):
                            pred_all_pass = False
                            break
                    ...
                    item = (
                        graph.state_name_string(state_to_show).ljust(37)
                        + " | "
                        + control_name
                    )
            ...
            self.title.configure(
                text="=====%s(%d%s)====="
                % (title_str, self.state_list.size(), number_suffix)
            )
    ...
    def __init__(
        self, graph: TwoPathGraph = None, graph_state: TwoPathGraph.StateId = None
    ):
        # initialize state
        ...
        self.canvas = tk.Canvas(
            self.main_window, width=400, height=400, background="white", name="canvas"
        )
        ...
        self.state_info_text_box = tk.Text(
            self.main_window, width=40, name="state_info_text_box"
        )
        ...
        self.state_go_to_entry = tk.Entry(
            self.main_window, width=40, name="state_go_to_entry"
        )
        ...
        self.state_go_to_button = tk.Button(
            self.main_window, text=">", height=1, width=2, name="state_go_to_button"
        )
        ...
        if self.gui_state.graph is not None:
            self.frs_brs_panel = TwoPathGraphViewer.FilterPanel(
                self.main_window, self.gui_state
            )
            self.frs_brs_panel.frame.grid(row=0, column=3, sticky=tk.NS)

            self.frs_brs_go_to_button = tk.Button(
                self.main_window,
                text="go to selected state",
    ...
    def go_to_state(self, state_id: TwoPathGraph.StateId):
        ...
        self.log_to_gui(
            "Navigated to:" + self.gui_state.graph.state_name_string(state_id)
        )
    ...
    def on_drag_canvas(self, e):
        if self.drag_canvas_start_pixel is None:
            ...
            self.gui_state.center_pos = (
                self.gui_state.center_pos[0]
                - dx * 1.0 / self.gui_state.pixel_per_meter,
                self.gui_state.center_pos[1]
                + dy * 1.0 / self.gui_state.pixel_per_meter,
            )
            self.drag_canvas_start_pixel = e.x, e.y

    def on_scroll_canvas(self, e):
        print(e.delta)
        tick = round(abs(e.delta) / 120.0)
        gain = 1.25 if e.delta > 0 else 0.8

        for _ in range(tick):
            self.gui_state.pixel_per_meter *= gain
        self.gui_state.pixel_per_meter = clamp(
            self.gui_state.pixel_per_meter,
            self._display_pixel_per_meter_min,
            self._display_pixel_per_meter_max,
        )
    ...
    def on_canvas_focus(self, e):
        ...
        a1_s_min = self.gui_state.graph.a1.discrete_dynamic.s_interval(
            a1_state[2]
        ).begin
    ...
    def pos2canvas(self, x, y) -> Tuple[int, int]:
        ...
        canvas_x = (
            x - xc
        ) * self.gui_state.pixel_per_meter + 0.5 * self.canvas.winfo_width()
        canvas_y = (
            -(y - yc) * self.gui_state.pixel_per_meter
            + 0.5 * self.canvas.winfo_height()
        )
    ...
    def update_canvas(self):
        ...
        def draw_single_vehicle(
            ...
            num_boxes = (
                max(
                    int((s_range.end - s_range.begin) / self._display_vehicle_pos_ds), 1
                )
                + 1
            )
        ...
        def draw_vehicles():
            ...
            draw_single_vehicle(
                a1.path, a1.vehicle, a1.discrete_dynamic, a1_sva_id, "#cc0000"
            )

            draw_single_vehicle(
                a2.path, a2.vehicle, a2.discrete_dynamic, a2_sva_id, "#0000cc"
            )
        ...
        def draw_single_path(
            ...
            num_s = (
                math.floor(
                    (self._display_path_length - self._display_path_offset)
                    / self._display_path_s_step
                )
                + 1
            )
            ...
            for i in range(num_s):
                ...
                polyline_canvas_pts.append(
                    self.pos2canvas(state[vehicle.X_ID], state[vehicle.Y_ID])
                )
        ...
        def draw_a2_sweep(color: str):
            if not hasattr(self.gui_state.graph, "a2_sweep_region_by_a1"):
                return
            a1_sva_id, a2_sva_id = self.gui_state.graph_state
            region = self.gui_state.graph.a2_sweep_region_by_a1(
                a1_sva_id[1], a2_sva_id[1], a2_sva_id[0]
            )
        ...
        def draw_regulation(
            reg_base: RegulationBase,
            binding_unsafe_pred: PredicateBase = None,
            y0=0,
            x0=0,
            height=100,
            width=100,
            ...
            dd1d = (
                self.gui_state.graph.a1.discrete_dynamic
                if reg_base.is_a1()
                else self.gui_state.graph.a2.discrete_dynamic
            )
            ...
            for a_ind in range(len(a_range)):
                for v_ind in range(len(v_range)):
                    ...
                    if (
                        binding_unsafe_pred is not None
                        and binding_unsafe_pred.evaluate(state_tpg)
                    ):
                        color = "#000000"
            ...
            self.canvas.create_text(
                x0, y0, anchor=tk.NW, text=reg_base.name(), fill="black", font=f
            )
            self.canvas.create_text(
                table_origin[0],
                table_origin[1],
        ...
        reg_tables_y0 = 20
        reg_tables_x0 = 20
        for name in self.gui_state.graph.sorted_regulation_names():
            reg_tables_y0 += draw_regulation(
                self.gui_state.graph.regulation(name),
                self.gui_state.graph.get_regulation_binding_unsafe_set(name),
                reg_tables_y0,
                reg_tables_x0,
            )
            reg_tables_y0 += 10
    ...
    def update_panel(self):
        ...
        state_debug_info = self.gui_state.graph.state_debug_info(
            self.gui_state.graph_state
        )
        self.state_info_text_box.insert(1.0, state_debug_info)
        self.state_info_text_box.configure(state="disabled")

        self.state_go_to_entry.delete(0, "end")
        self.state_go_to_entry.insert(
            0, self.gui_state.graph.state_name_string(self.gui_state.graph_state)
        )
